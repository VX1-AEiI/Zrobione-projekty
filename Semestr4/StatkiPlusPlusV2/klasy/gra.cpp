...
void Gra::rozgrywkaMenuGracza()
{
    std::string wybor {""};
    std::string nazwaGracza {""};
    std::regex forceQuit("EXIT");
    if (g1g2)
        nazwaGracza = Gracz1.getImie();
    else
        nazwaGracza = Gracz2.getImie();
...
std::string Gra::menuAtakuResponse()
{
    std::string tekst{""};
    char wiersz {'w'};
    char kolumna {'w'};
...
bool Gra::menuAtakuGracz1()
{
    ...
    std::string odp = menuAtakuResponse();
    if (odp != "ww")
    {
